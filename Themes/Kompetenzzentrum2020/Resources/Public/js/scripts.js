...
(function ($, window, document, undefined) {
  ...
  $.extend(Plugin.prototype, {
    init: function () {
      ...
      plugin.settings.$accordionButtons = $(".accordion__button");
    ...
    bindEvents: function () {
      plugin.settings.$accordionButtons.on("click", plugin.accordionToggle);
      plugin.settings.$accordionButtons.on(
        "keydown",
        plugin.keyboardInputManagement
      );
    },
    ...
    accordionToggle: function (e) {
      ...
      var $control = $(this); //Returns an object that contains the clicked button html element

      var accordionContent = $control.attr("aria-controls");
      plugin.checkOthers($control[0]); //$control[0] returns button html element

      //Set aria hidden value to opposite value (true/false)
      isAriaHid = $("#" + accordionContent).attr("aria-hidden");
      if (isAriaHid == "true") {
        $("#" + accordionContent).attr("aria-hidden", "false");
        $("#" + accordionContent).slideDown({ duration: 500, queue: false });
      } else {
        $("#" + accordionContent).attr("aria-hidden", "true");
        $("#" + accordionContent).slideUp({ duration: 500 });
      }
    ...
    //elem : button html element
    checkOthers: function (elem) {
    ...
    keyboardInputManagement: function (e) {
      ...
      var accordionButtons = plugin.settings.$accordionButtons;
      if (e.key == "ArrowUp") {
        var value = plugin.findAccordionButtonIndex(e.target);
        if (value != -1) {
          if (value == 0) {
...
(function ($, window, document, undefined) {
  ...
  var pluginName = "dropdownNavbar",
    defaults = {
      toggleClass: ".js-toggle-dropdown",
  ...
  $.extend(Plugin.prototype, {
    init: function () {
      ...
      plugin.settings.$el = $(this.element);
      plugin.settings.$toggleableElements = plugin.settings.$el.find(
        plugin.settings.toggleClass
      );
    ...
    // Bind all event listeners for this plugin
    bindEvents: function () {
      plugin.settings.$el.on("mouseover", plugin.hoverIn);
      plugin.settings.$el.on("mouseout", plugin.hoverOut);
    },

    hoverIn: function (e) {
      $(this)
        .find(plugin.settings.toggleClass)
  ...
  $.extend(Plugin.prototype, {
    ...
    open: function (e) {
      ...
      if (plugin.settings.viewport.length <= 0) return;
        ...
        if (
            bottom_of_screen > plugin.settings.viewport.top &&
            top_of_screen < plugin.settings.viewport.bottom &&
            ! plugin.settings.$footer.hasClass(plugin.settings.isHidden)
        ) {
    ...
    close: function () {
      plugin.settings.$footer.removeClass(plugin.settings.isActive);
      plugin.settings.$footer.addClass(plugin.settings.isHidden);
    },
 ...
 *
 * Author: helllicht, Steffen Kroggel <developer@steffenkroggel.de>
 *
...
(function ($, window, document, undefined) {
  ...
  var pluginName = "mobileNavbar",
    defaults = {
      ...
      navMobileItemClass: ".nav-mobile__item--toggle",
  ...
  $.extend(Plugin.prototype, {
    init: function () {
      ...
      plugin.settings.itemsInitalized = false;
    ...
    initItems: function () {
      ...
      plugin.settings.itemsInitalized = true;
    ...
    // Bind all event listeners for this plugin
    bindEvents: function () {
      if (plugin.settings.itemsInitalized) {
    ...
    adjustElements: function () {
      ...
      if (plugin.settings.itemsInitalized) {
        ...
        var activeListDiv = plugin.settings.$navLists
          .filter("." + plugin.settings.isActive)
          .last();
        var activeListUl = activeListDiv.children()
          .filter("ul")
          .first();
